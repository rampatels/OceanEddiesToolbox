function plot1typeTrack(tracks, filter_day, coast, xtick, ytick)
% if you want number of eddy in your particuler xtick and ytick then you
% should filter your tracks first and then apply this function otherwise
...
% input agrument
if (nargin ~= 5)
   error('plot1typeTrack.m: Must pass 5 parameters')
end
if (~iscell(tracks))
    error('plot1typeTrack.m: Tracks data must be in cell')
end
%  Filtering criterion
if (numel(filter_day) ~= 1)
    error('plot1typeTrack.m: only single filter criterion at a time')
end
...
for ind = 1:length(c_age)
    ...
    hold on
    plot(lon(1), lat(1),'ob','MarkerFaceColor','b','Markersize',5,'MarkerEdgecolor','k') % Birth Place : 1, dissipation end
end
