function [shape, fitshape] = ra_get_eddyshapes(eddies, eddyind, data_lon, data_lat, R)
...
% shape = cell(length(eddies(eddyind)),1);
% fitshape = cell(length(eddies(eddyind)), 1);
%
oceanmask = zeros(length(data_lat), length(data_lon));
pixellist = eddies(eddyind).Stats.PixelIdxList;
...
shape = [data_lon(boundary(:, 2)), data_lat(boundary(:, 1))]; % the pixelled shape
% fitting an ellipse to pixelled shape
a = eddies(eddyind).Stats.MajorAxisLength/2; % major axis
b = eddies(eddyind).Stats.MinorAxisLength/2; % minor axis
theta = atan(-eddies(eddyind).Stats.Orientation*(pi/180)); % rotated to fix an issue, orientation
% ellipse contour
...
fitshape = [lon_fit', lat_fit']; % the fitted ellipse
